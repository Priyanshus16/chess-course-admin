...
export default function Users() {
  ...
  const getData = async() => {
    const response = await axios.get(`http://localhost:4000${ADMIN_API_PREFIX}/users`);
    setApiData(response.data.users);
  }

  const handleUserDelete = async(id) => {
    try {
      const response = await axios.delete(`http://localhost:4000/${ADMIN_API_PREFIX}/users/${id}`)
